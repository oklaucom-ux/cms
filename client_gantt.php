...
if (!$project_id) die("<div class='content-section active'><h2>No Project Specified</h2></div>");

// Ensure the client owns it if they are not Admin
if ($_SESSION['role'] === 'Client') {
    $stmt = $pdo->prepare("SELECT * FROM projects WHERE id = ? AND (client_id = ? OR client = ?)");
    $stmt->execute([$project_id, $_SESSION['login_id'], $_SESSION['name']]);
} else {
...
if (empty($projects)) {
    die("<div class='content-section active'><h2>Project not found or access denied.</h2><a href='client_portal.php'>Return to Portal</a></div>");
}
