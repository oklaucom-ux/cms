...
if ($isAdmin) {
    // ΓöÇΓöÇ Consolidated Admin Metrics (single query replaces 10 individual COUNTs) ΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ Phase 18 KPIs (consolidated into 1 query) ΓöÇΓöÇ
    try {
        $p18 = $pdo->query("
            SELECT
    ...
    // ΓöÇΓöÇ CHART DATA: Revenue by Month (last 6 months) ΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ CHART DATA: Pipeline by Stage ΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ CHART DATA: Omni-Channel Tickets ΓöÇΓöÇ
    ...
    // ΓöÇΓöÇ CHART DATA: Activity Bar Chart (Last 7 Days) ΓöÇΓöÇ
    $activityLabels = [];
    $activityData = [];
    try {
        global $use_mysql;
...
<div class="content-section active" id="printableDashboard">
    ...
    <div style="background: linear-gradient(to right, #1e3a8a, #3b82f6); border-radius: 12px; padding: 20px; margin-bottom: 25px; color: white; box-shadow: 0 4px 6px rgba(0,0,0,0.1);" data-html2canvas-ignore="true">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 15px;">
            <h3 style="margin:0; font-size: 18px; display:flex; align-items:center; gap:8px;">≡ƒôú Official Announcements</h3>
    ...
    <div style="background: rgba(239, 68, 68, 0.1); border: 1px solid #ef4444;  padding: 20px; border-radius: 8px; margin-bottom: 25px;" data-html2canvas-ignore="true">
        <h3 style="color: #b91c1c; margin-bottom: 10px; display:flex; align-items:center; gap:10px;">ΓÜá∩╕Å Urgent Action Required</h3>
    ...
    <?php if($isAdmin): ?>
        <div class="section-header" data-html2canvas-ignore="true">
            ...
            <button class="add-button" onclick="generatePDF()">≡ƒôÑ Export Report to PDF</button>
        </div>
        ...
        <h3 style="color:var(--text-heading);font-size:18px;font-weight:700;margin:32px 0 16px;letter-spacing:-0.5px;">≡ƒÜÇ Enterprise Operations ΓÇö Live</h3>
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:16px;margin-bottom:32px;">
            ...
            <div class="glass-card hoverable" style="padding:20px;">
                ...
                <div style="font-size:12px;color:var(--text-muted);margin-top:6px;"><?= ($GLOBAL_SETTINGS['currency'] ?? 'Γé╣') ?><?= number_format($p18_totalSpent) ?> spent</div>
            </div>
            <div class="glass-card hoverable" style="padding:20px;">
                ...
                <div style="font-size:34px;font-weight:800;color:#ef4444;margin-top:4px;"><?= ($GLOBAL_SETTINGS['currency'] ?? 'Γé╣') ?><?= number_format($p19_unpaidInvoices) ?></div>
            ...
            <div class="glass-card hoverable" style="padding:20px;">
                ...
                <div style="font-size:28px;font-weight:800;color:#10b981;margin-top:4px;"><?= ($GLOBAL_SETTINGS['currency'] ?? 'Γé╣') ?><?= number_format($p18_pipelineValue) ?></div>
        ...
        <h3 style="color:var(--text-heading);font-size:18px;font-weight:700;margin:32px 0 16px;letter-spacing:-0.5px;">≡ƒÆ╝ Business & HR Overview</h3>
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:16px;margin-bottom:32px;">
            <div style="background:var(--bg-card);border-radius:16px;padding:20px;border:1px solid var(--border-card);box-shadow:var(--shadow-soft);">
                ...
                <div style="font-size:28px;font-weight:800;color:#ef4444;margin-top:4px;"><?= ($GLOBAL_SETTINGS['currency'] ?? 'Γé╣') ?><?= number_format($p19_unpaidInvoices) ?></div>
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:24px; margin-bottom:24px;">
            ...
            <div style="background:var(--bg-card); padding:24px; border-radius:16px; box-shadow:var(--shadow-soft); border:1px solid var(--border-card);">
                <h4 style="margin:0 0 20px; color:var(--text-heading); font-size:1.1rem;">≡ƒôê Invoice Revenue ΓÇö 6 Months</h4>
                <div style="height:250px;"><canvas id="revenueChart"></canvas></div>
            </div>
            <!-- Pipeline Doughnut -->
            <div style="background:var(--bg-card); padding:24px; border-radius:16px; box-shadow:var(--shadow-soft); border:1px solid var(--border-card);">
                <h4 style="margin:0 0 20px; color:var(--text-heading); font-size:1.1rem;">≡ƒÄ» CRM Pipeline</h4>
            ...
            <div style="background:var(--bg-card); padding:24px; border-radius:16px; box-shadow:var(--shadow-soft); border:1px solid var(--border-card);">
                <h4 style="margin:0 0 20px; color:var(--text-heading); font-size:1.1rem;">≡ƒÄ½ Omni-Channel Tickets</h4>
