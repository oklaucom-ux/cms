...
function cwLoadContacts() {
    // We can fetch contacts by hitting a custom API or a new action.
    // Let's create an action in chat_api or a mini fetch file.
    // Wait, chat_api doesn't have an action to fetch just the allowed contacts for clients.
    // We will build a small dedicated endpoint or we can query it here using a mini php script.
...
function cwPollUnread() {
    if (cwActivePartner) return; // don't poll global unreads if inside a thread
...
// Start polling immediately for badge counts
setTimeout(() => {
    cwLoadContacts();
    cwPollInterval = setInterval(cwPollUnread, 5000);
}, 1000);
